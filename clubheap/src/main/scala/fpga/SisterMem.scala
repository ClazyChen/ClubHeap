...
class SisterMem(
    ...
    if (no_sister) {
        io.rdata_lc_out := mem_lc.read(io.raddr_in, io.ren_in)
        io.rdata_rc_out := DontCare
        
        mem_lc.write(io.waddr_in, io.wdata_lc_in, io.wen_in)
    } else {
        // the memory of the right child
        val mem_rc = generate_memory()
        io.rdata_lc_out := mem_lc.read(io.raddr_in, io.ren_in)
        io.rdata_rc_out := mem_rc.read(io.raddr_in, io.ren_in)
        
        mem_lc.write(io.waddr_in, io.wdata_lc_in, io.wen_in)
        mem_rc.write(io.waddr_in, io.wdata_rc_in, io.wen_in)
    }
}
